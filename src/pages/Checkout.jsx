...
const initialAddress = { cep: '', street: '', number: '', complement: '', neighborhood: '', city: '', state: '' }
...
export default function Checkout({ cart, payment, setPayment, onConfirm }) {
  ...
  return <main className="container checkout">
    ...
    <div className="checkout-grid"><section>
      <AddressFields title="📍 Endereço de entrega" description="Digite o CEP para preencher rua, bairro, cidade e UF automaticamente." value={address} onChange={setAddress} />
      <article className="form-card"><h3>💳 Forma de pagamento</h3><div className="payment">{['Cartão de crédito','PIX','Boleto'].map(method => <button key={method} type="button" className={payment === method ? 'active' : ''} onClick={() => setPayment(method)}>{method === 'PIX' ? '📱' : method === 'Boleto' ? '🏦' : '💳'}<br />{method}</button>)}</div>
