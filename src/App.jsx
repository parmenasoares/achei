...
export default function App() {
  ...
  const visible = useMemo(() => catalogProducts.filter(product => {
    ...
    const expandedSearch = normalize(deferredQuery).split(/\s+/).filter(Boolean).flatMap(term => [term, ...(synonyms[term] || '').split(' ')]).filter(Boolean)
    ...
    return (category === 'Todos' || product.category === category)
      ...
      && text.includes(normalize(vehicleFilters.part))
  ...
  }).sort((a, b) => sort === 'price-asc' ? a.price - b.price : sort === 'price-desc' ? b.price - a.price : sort === 'rating' ? b.rating - a.rating : 0), [catalogProducts, deferredQuery, category, sort, vehicleFilters])
